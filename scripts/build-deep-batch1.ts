...
// Helper to assemble full HTML body with all rich sections
function generateDeepHtml(c: DeepConfig): string {
  const tableHtml = c.tableData
    ? `
<h2 id="matriks-checklist">Matriks Checklist &amp; Standar Verifikasi Lapangan</h2>
<div class="table-scroll"><table>
  <tr>${c.tableData.headers.map((h) => `<th>${h}</th>`).join('')}</tr>
  ${c.tableData.rows.map((row) => `<tr>${row.map((cell) => `<td>${cell}</td>`).join('')}</tr>`).join('\n  ')}
</table></div>`
    : '';

  return `<p>${c.intro}</p>

<h2 id="analisis-risiko">Identifikasi Bahaya &amp; Manajemen Risiko Operasional</h2>
<p>Kegagalan dalam mematuhi prosedur operasi standar pada program ini memicu dampak fatalitas tinggi, kerusakan aset mekanikal masif, dan penghentian lini produksi. Beberapa potensi bahaya kritis meliputi:</p>
<ul class="check-list">
  ${c.hazards.map((h) => `<li>${h}</li>`).join('\n  ')}
</ul>

<h2 id="landasan-hukum">Kerangka Hukum &amp; Standar Regulasi Ketenagakerjaan</h2>
<p>Pelatihan ini disusun mengacu pada standar regulasi nasional dan internasional yang berlaku di Indonesia:</p>
<ul>
  ${c.regulations.map((r) => `<li>${r}</li>`).join('\n  ')}
</ul>
...
<p>Peserta pelatihan dibekali pengalaman langsung mengoperasikan instrumen dan alat keselamatan standar industri:</p>
<ul class="check-list">
  ${c.equipment.map((e) => `<li>${e}</li>`).join('\n  ')}
</ul>

<h2 id="kriteria-peserta">Profil Sasaran Peserta Pelatihan</h2>
<p>Program in-house dan sertifikasi ini direkomendasikan bagi:</p>
<ul>
  ${c.participants.map((pt) => `<li>${pt}</li>`).join('\n  ')}
</ul>

<h2 id="evaluasi-sertifikasi">Metodologi Evaluasi &amp; Uji Kompetensi</h2>
<p>Penilaian kelulusan peserta dilakukan secara objektif melalui ujian teori tertulis (Pre-Test &amp; Post-Test) dengan ambang batas nilai kelulusan minimal, observasi langsung pada uji praktik lapangan, serta verifikasi portofolio kerja. Peserta yang dinyatakan kompeten akan memperoleh Sertifikat Resmi Kualifikasi K3.</p>

<h2 id="program-terkait">Navigasi Program Terkait &amp; Lanjutan</h2>
<p>Perkuat integrasi kompetensi keselamatan tim operasional fasilitas Anda melalui program pilihan:</p>
<p>
...
async function runUpgrade() {
  console.log('====================================================');
  console.log('🚀 UPGRADING BATCH 1 PAGES WITH COMPREHENSIVE EDITORIAL DEPTH');
  console.log('====================================================\n');
  ...
  for (const cfg of deepConfigs) {
    const html = generateDeepHtml(cfg);
    const contentPath = path.join(CONTENT_DIR, `${cfg.key}.ts`);
    const code = `import { PageContentData } from '@/lib/types';

export const content: PageContentData = {
  key: ${JSON.stringify(cfg.key)},
  updated: "2026-08-29",
  faq: ${JSON.stringify(cfg.faqs, null, 2)},
  html: ${JSON.stringify(html)}
};
`;
    fs.writeFileSync(contentPath, code, 'utf8');
    console.log(`✅ [Upgraded Deep Content]: ${cfg.key}.ts`);

    const qIdx = topicQueue.sections.live.findIndex((p) => p.key === cfg.key);
    if (qIdx !== -1) {
      topicQueue.sections.live[qIdx].title = cfg.title;
      topicQueue.sections.live[qIdx].h1 = cfg.h1;
      topicQueue.sections.live[qIdx].meta = cfg.meta;
      topicQueue.sections.live[qIdx].img_alt = cfg.imgAlt;
    }
  }

  fs.writeFileSync(QUEUE_FILE, JSON.stringify(topicQueue, null, 2), 'utf8');
  console.log('\n🎉 Topic Queue and deep content files upgraded!');
}
