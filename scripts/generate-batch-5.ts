...
async function runBatch5Generator() {
  console.log('====================================================');
  console.log('🚀 GENERATING BATCH 5 (35 INDUSTRIES, REGIONAL HUBS & STANDARDS)');
  console.log('====================================================\n');
  ...
  for (let i = 0; i < batch5List.length; i++) {
    const data = batch5List[i];
    const prevKey = i > 0 ? batch5List[i - 1].key : 'tekstil-garmen-dan-sepatu';
    const nextKey = i < batch5List.length - 1 ? batch5List[i + 1].key : 'manajemen-sdm';
    ...
    const pageMeta: PageMeta = {
      ...
      img_alt: data.imgAlt,
      status: 'live',
      batch: 'batch-5-industries-regional-hubs-standards',
    };
...
`;
  ...
  console.log(`\n🎉 Stored ${batch5List.length} Batch 5 pages in topic-queue.json (Total Live: ${topicQueue.summary.total_live})`);
...
runBatch5Generator().catch((err) => {
  console.error('Error generating Batch 5:', err);
  process.exit(1);
});
