...
async function runBatch2Generator() {
  console.log('====================================================');
  console.log('🚀 GENERATING BATCH 2 (33 ELECTRICAL, MECHANICAL, FIRE & HAZMAT PAGES)');
  console.log('====================================================\n');
  ...
  for (let i = 0; i < batch2List.length; i++) {
    const data = batch2List[i];
    const prevKey = i > 0 ? batch2List[i - 1].key : 'permit-to-work';
    const nextKey = i < batch2List.length - 1 ? batch2List[i + 1].key : 'manajemen-sdm';
    ...
    const pageMeta: PageMeta = {
      ...
      img_alt: data.imgAlt,
      status: 'live',
      batch: 'batch-2-electrical-mechanical-fire-hazmat',
    };
...
`;
  ...
  console.log(`\n🎉 Stored ${batch2List.length} Batch 2 pages in topic-queue.json (Total Live: ${topicQueue.summary.total_live})`);
...
runBatch2Generator().catch((err) => {
  console.error('Error generating Batch 2:', err);
  process.exit(1);
});
